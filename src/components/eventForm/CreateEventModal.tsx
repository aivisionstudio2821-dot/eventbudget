...
const PRIORITIES: { id: Priority; label: string; desc: string; icon: string }[] = [
  { id: 'Balanced', label: 'Balanced Plan', desc: 'Equal distribution across all categories', icon: '⚖️' },
  { id: 'Food', label: 'Food & Dining Focus', desc: 'Top tier catering, live counters & desserts', icon: '🍽️' },
  { id: 'Venue', label: 'Grand Venue Focus', desc: 'Premium banquet hall or sprawling lawns', icon: '🏰' },
  { id: 'Decoration', label: 'Aesthetic Decor Focus', desc: 'Floral arches, theme backdrop & lighting', icon: '✨' },
  { id: 'DJ / Music', label: 'DJ & Sound Focus', desc: 'High-energy sound, moving lights & concert vibes', icon: '🎧' },
  { id: 'Photography', label: 'Photography Focus', desc: 'Candid shots, drone & Instagram reels crew', icon: '📸' },
];
...
export const CreateEventModal: React.FC<CreateEventModalProps> = ({
  ...
  // Tomorrow's date as default min
  ...
  const [eventDate, setEventDate] = useState<string>(defaultDateStr);
  ...
  const validate = (): boolean => {
    ...
    if (!totalBudget || isNaN(budgetNum) || budgetNum <= 0) {
      errs.totalBudget = 'Please enter a valid positive budget amount.';
    } else if (budgetNum < 5000) {
      errs.totalBudget = 'Minimum realistic event budget is ₹5,000.';
    }

    const guestNum = Number(guestCount);
    if (!guestCount || isNaN(guestNum) || guestNum <= 0) {
      errs.guestCount = 'Guest count must be at least 1 person.';
    }

    if (!city.trim()) {
      errs.city = 'Please enter your event city / area.';
    }

    if (!eventDate) {
      errs.eventDate = 'Please select a valid event date.';
    } else {
      ...
      <div className="relative w-full max-w-2xl bg-[#0f172a] border border-slate-700/80 rounded-3xl shadow-2xl overflow-hidden my-6">
        ...
        <div className="relative px-6 py-5 border-b border-slate-800 bg-gradient-to-r from-purple-950/40 via-slate-900 to-slate-900 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-xl font-bold text-white font-heading">Plan Your Celebration</h2>
              <p className="text-xs text-slate-400">Set your budget & let our Smart Engine allocate every rupee</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        {/* Form Body */}
        <form onSubmit={handleCreate} className="p-6 sm:p-8 space-y-6 max-h-[80vh] overflow-y-auto">
          ...
          <div className="p-3.5 rounded-2xl bg-amber-500/10 border border-amber-500/30 flex items-center justify-between gap-3">
            <div className="text-xs text-amber-200">
              <span className="font-bold">Judge / Fast Demo?</span> Load our pre-configured ₹50,000 Ahmedabad Birthday Plan in 1-click.
            </div>
            <button
              type="button"
              onClick={() => { onClose(); onLoadDemo(); }}
          ...
          <div>
            <label className="block text-xs font-bold uppercase tracking-wider text-slate-300 mb-2">
              1. Event Type <span className="text-rose-400">*</span>
            </label>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-xs font-bold uppercase tracking-wider text-slate-300 mb-1.5">
                2. Total Budget (₹) <span className="text-rose-400">*</span>
              </label>
              ...
              {errors.totalBudget ? (
                ...
                <p className="mt-1 text-[11px] text-slate-500">Suggested: ₹20,000 to ₹10,00,000</p>
              )}
            </div>
            ...
            <div>
              <label className="block text-xs font-bold uppercase tracking-wider text-slate-300 mb-1.5">
                3. Number of Guests <span className="text-rose-400">*</span>
              </label>
              <div className="relative">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-xs font-bold uppercase tracking-wider text-slate-300 mb-1.5">
                4. City / Area <span className="text-rose-400">*</span>
              </label>
              <div className="relative">
              ...
              {errors.city ? (
                ...
                <p className="mt-1 text-[11px] text-slate-500">Default demo market: Ahmedabad</p>
              )}
            </div>
            ...
            <div>
              <label className="block text-xs font-bold uppercase tracking-wider text-slate-300 mb-1.5">
                5. Event Date <span className="text-rose-400">*</span>
              </label>
              <div className="relative">
          ...
          <div>
            <label className="block text-xs font-bold uppercase tracking-wider text-slate-300 mb-2">
              6. Main Priority (Directs Smart Allocation) <span className="text-rose-400">*</span>
            </label>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
              {PRIORITIES.map((p) => (
                ...
                >
                  ...
                  <div>
                    <p className={`text-xs font-bold ${priority === p.id ? 'text-purple-300' : 'text-slate-200'}`}>
                      {p.label}
                    </p>
          ...
          <div className="pt-4 border-t border-slate-800 flex flex-col sm:flex-row items-center justify-end gap-3">
            ...
            <button
              type="submit"
              className="w-full sm:w-auto px-7 py-3.5 rounded-xl font-bold text-sm text-white bg-gradient-to-r from-purple-600 via-pink-600 to-purple-600 hover:from-purple-500 hover:to-pink-500 shadow-lg shadow-purple-600/30 transition-all flex items-center justify-center gap-2 active:scale-95"
            >
