...
/**
 * Calculates dynamic initial allocations based on event type,
 * guest count, total budget, and priority.
 */
export function calculateSmartAllocations(
  ...
  const normalizedKeys: CategoryKey[] = [
    'food',
    'venue',
    'decoration',
    'dj',
...
export function calculateDecorTotal(
  ...
  const appliedQuoteId =
    state.appliedQuoteIds?.decoration;
...
export function calculatePhotoTotal(
  ...
  const appliedQuoteId =
    state.appliedQuoteIds?.photography;
...
export function calculateCategoryTotals(
  ...
  return {
    food:
      calculateFoodTotal(state),

    venue:
      calculateVenueTotal(state),

    decoration:
      calculateDecorTotal(state),

    dj:
      calculateDJTotal(state),

    photography:
      calculatePhotoTotal(state),

    misc:
      calculateMiscTotal(state),
...
export function getSmartAlternatives(
  ...
  if (
    ...
    suggestions.push({
      ...
      description:
        `Save ₹${savePerPerson}/person ` +
        `(Total savings: ₹${totalSavings.toLocaleString(
          'en-IN'
        )}) with a crowd-favorite classic starter.`,
  ...
  if (
    ...
    suggestions.push({
      ...
      title:
        'Switch Signature Mocktails to Chilled Soft Drinks',

      description:
        `Save ₹${totalSavings.toLocaleString(
          'en-IN'
        )} by serving refreshing soft drinks instead of custom mocktails.`,
  ...
  if (
    ...
    suggestions.push({
      ...
      title:
        'Switch North Indian Buffet to Punjabi Meal Platter',

      description:
        `Save ₹${totalSavings.toLocaleString(
          'en-IN'
        )} without compromising on rich taste and satisfaction.`,
  ...
  if (
    ...
    suggestions.push({
      ...
      title:
        'Switch Floral Decor to Custom 3D Backdrop',

      description:
        'Save ₹6,500 by using modern 3D theme backdrop styling instead of fresh imported florals.',
